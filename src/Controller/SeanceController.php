...
final class SeanceController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $seance = $this->generateData($seance);

            $entityManager->persist($seance);
            $entityManager->flush();

            return $this->redirectToRoute('app_seance_show', [
                'id' => $seance->getId()
    ...
    public function generateData(Seance $seance): Seance
    {
        $seance->setDuration(random_int(30, 120));
        $seance->setPercievedIntensity(random_int(1, 10));
        $seance->setBurnedCalories(random_int(100, 1200));
        $seance->setBodyTemperatureDelta(random_int(-50, 50) / 10);
        $seance->setMuscleFatigue(random_int(0, 100));
        $seance->setHeartRecoveryRate(random_int(5, 30));
        $seance->setHeartbeatVariation(random_int(15, 55));
